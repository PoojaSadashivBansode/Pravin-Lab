...
import { Menu, X, User, FileText, Calendar, LogOut, ChevronDown, LogIn, UserPlus } from "lucide-react"; 
...
const Navbar = () => {
  ...
  // Set this to true when a user logs in. Default is false (Logged Out).
                  ...
                  <div className="absolute right-0 mt-3 w-60 bg-white border border-gray-100 rounded-2xl shadow-xl py-2 z-50 overflow-hidden">
                    ...
                    {isLoggedIn ? (
                      ...
                      <>
                        ...
                        <Link to="/register" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors font-medium">
        ...
        <div className="md:hidden bg-white border-t border-brand-red px-6 py-6 space-y-5">
          ...
          <div className="pt-4 border-t border-gray-100">
             {isLoggedIn ? (
               ...
               <Link to="/login" onClick={() => setIsOpen(false)} className="block text-lg font-bold text-brand-blue">Login / Sign Up</Link>
             )}
          </div>
        </div>
      )}
